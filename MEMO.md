...
**Final submission pass** hardened the slice so the same pipeline lights up for **any vs show**, not just Coastal-shaped emails, and so the worksheet is honestly traceable line-by-line: deal-record fallbacks when the regex parser misses, a generic `Ambiguity` loop in the confirm form, per-row cap-status badges (`pre-cap` / `in cap` / `cap binds` / `at cap` / `within cap` / `absorbed`), a running balance under every amount, and an explicit "in-cap bucket → cap" row that's emitted even at boundary cases. 18 tests assert the engine math and the transparency shape.
...
**F2 — Auditable statement.** The settle page renders an `AuditableWorksheet` for vs deals. What's on screen: confirmed-terms banner ("Calculated against confirmed deal terms — $5,000 vs 80%, recoup off gross") with a `View terms →` link back to F0; per-line source badges (`deal` / `pos` / `receipt` / `manual` / `absorbed`); per-line **cap-status badges** (`pre-cap`, `in cap`, `cap binds`, `at cap`, `within cap`, `absorbed`); a **running balance** under each amount so the reader can see exactly where the math sits after every line; an explicit **"In-cap bucket subtotal"** row (`expenses $X + in-cap deductions $Y = $Z`) followed by an always-on cap row that distinguishes `cap_binding` (savings > 0), `cap_at` (bucket exactly equals cap — both readings collapse here), and `cap_within` (cap doesn't bind); absorbed-by-venue section surfaced separately so the agent sees costs the venue ate; explicit guarantee-comparison row ("Artist share $X vs guarantee $Y — paying the [winner]"); and a **counterfactual block** when the other recoup reading would have paid a different total ("If the team had picked recoup in cap, the total would be $13,484.80 — a $720 higher payout"). The `calculation_json` is the durable artifact the North Star measures.

**Phase A widening — any vs show settles, not just Coastal-shaped ones.** The form pipeline was deliberately narrow in the first cut (one hard-coded `marketing_recoup` field + one hard-coded ambiguity card). Final pass: `Ambiguity.field` is a generic dotted path with `string` option values; the confirm form loops over `parsed.ambiguities` and builds the engine's `deductions[]` from `parsed.extracted.deductions` with per-id resolutions, so a future deduction type (hospitality overage, production overage) lights up the same UI without touching the form. `mergeDealRecordFallbacks(parsed, deal)` fills `guarantee_amount`, `artist_percent`, `expense_cap`, and gross-threshold rows from `deal.bonusesJson` whenever the regex parser missed — many seeded vs deals only carry numbers on the deal row, not in the prose, and they now confirm without manual re-entry. The engine accepted multiple deductions before this pass; the form just couldn't produce them. Now it can.
...
7. **Generic ambiguities, not a Coastal-only special case.** It would have been quicker to hard-code `RECOUP_AMBIGUITY_FIELD` in the form and ship. I widened the `Ambiguity` contract instead — `field: string`, `options[].value: string` — and made the form a generic loop. Reasoning: F0's value is forced choice at deal entry, not specifically forced choice about marketing recoup. The next ambiguity type (e.g. "is this bonus on gross or net?") should add a new ambiguity emitter in the parser, not a new card in the form. The same logic drove a generic per-id `deductionAmounts` map in form state — one `if (id === "marketing_recoup")` branch averted now is a refactor avoided when the second deduction type lands.

8. **Worksheet transparency: line-by-line traceable beats final-number-correct.** Math correctness is table stakes (golden tests assert $11,564.80 / $12,284.80 / $720 to the cent). The harder question is whether a TM can read the worksheet at 2am and answer Diego's questions without asking Mariana. Every deduction-phase row now carries `runningBalance` (the literal balance after applying that row) and a `capStatus` badge. The cap row is emitted even when savings = 0 — silently skipping it at the boundary (Coastal's exact case) is the kind of "where did the cap go?" hole that erodes trust. Cost: 5 new tests, ~80 lines of UI, ~40 lines of engine. Cheap relative to the dispute-prevention value.
